...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print("WARNING: weather_master_v3.pkl not found. ML inference will fall back to 0.")
    yield
...
class AirQuality(BaseModel):
    ...
    so2_ugm3:           float | None
    o3_ugm3:            float | None
    co_mgm3:            float | None     # converted from µg/m³ → mg/m³
...
class RiskAssessment(BaseModel):
    ...
    final_verdict:        str            # FIX: was "Final_verdict" — snake_case
...
def predict_ml_risk(w: dict) -> int:
    ...
    features = pd.DataFrame([{
        ...
        "Temp_Fluctuation":    w["temp_max"] - w["temp_mean"],
...
def get_dominant_pollutant(a: dict) -> str:
    """EPA sub-index method — highest sub-index pollutant wins."""
    def pm25_to_aqi(c):
    ...
    def o3_to_aqi(c):
        # Open-Meteo gives µg/m³; rough ppb conversion (/2) for 8-hr EPA table
        c_ppb = c / 2
    ...
    try:
        ...
        if a["no2"]  is not None:
            v = a["no2"]   # µg/m³; simplified linear bracket
        ...
        if a["co"]   is not None:
            # a["co"] is already mg/m³ at this point (converted during fetch)
            v = a["co"]
...
def build_advisory(w: dict, a: dict, final_risk: int) -> list[str]:
    """
    Tiered advisory — each section is only triggered by its own condition.
    SAFE/MODERATE/HIGH RISK base advice is always present.
    Temperature, AQI, pollutant, and precipitation sections are independent.
    """
    temp    = w["temp_mean"]
    aqi_val = a["aqi"] if a["aqi"] is not None else 0.0   # FIX: explicit None check
    bullets: list[str] = []

    # ── Section 1: Overall status (always shown) ────────────────────────────
    if final_risk == 0:
        bullets.extend([
    ...
    # ── Section 2: Temperature (independent of overall risk) ────────────────
    # FIX: temperature advice shown regardless of final_risk — a SAFE city can
    # still be 42°C, which is a real physical danger the user must know about.
    if temp > 40:
        bullets.extend([
    ...
    # ── Section 3: AQI tier (only shown if AQI is meaningful) ───────────────
    if aqi_val > 300:
        bullets.extend([
    ...
    # ── Section 4: Pollutant-specific (only if AQI is elevated) ─────────────
    if aqi_val > 50:
    ...
    # ── Section 5: Precipitation (only if raining) ───────────────────────────
...
class AsyncGlobalWeatherEngine:
    ...
    async def fetch_metrics(lat: float, lon: float) -> tuple[dict, dict]:
        async with httpx.AsyncClient() as client:
            # FIX: pass coroutines directly to gather, not pre-called responses
        ...
        # FIX: parse each response exactly once
        w_json = w_res.json()
        a_json = a_res.json()
        cw     = w_json["current"]
        ca     = a_json["current"]
        ...
        aqi = {
            ...
            "o3":   num(ca.get("ozone")),
            "co":   round(co_raw / 1000, 3) if co_raw is not None else None,  # µg/m³ → mg/m³
        }
...
def extract_city(text: str) -> str:
    clean = re.sub(
        r"(?i)\b(what|whats|weather|in|is|how|show|me|city|aqi|of|for|tell|about)\b",
        "", text,
    ).strip()
    if nlp:
        doc  = nlp(clean)
...
@app.post(
    ...
    description="Fetches real-time weather and AQI data, processes it through the ML engine, and generates a detailed JSON advisory report."
)
async def analyze(
    query: str = Query(
        ..., 
